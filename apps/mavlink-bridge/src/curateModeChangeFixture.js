...
if (!sourcePath || !destinationPath) {
  console.error('usage: node curateModeChangeFixture.js SOURCE.jsonl DESTINATION.jsonl')
  process.exit(2)
}

const events = fs.readFileSync(sourcePath, 'utf8').split('\n').filter(Boolean)
  .map((line) => JSON.parse(line))
  .filter((entry) => entry.event?.type === 'modeChange')

const completed = events.filter((entry) => entry.event.status === 'complete')
if (completed.length !== 4 || !completed.some((entry) => entry.event.sysId === 1)
  || !completed.some((entry) => entry.event.sysId === 2)) {
  throw new Error('source must contain four completed Copter/Plane mode transitions')
}

const requestIds = new Map(), requestTimes = new Map()
let nextRequest = 1
const baseMs = 1_800_000_200_000
const curated = events.map((entry, index) => {
  if (!requestIds.has(entry.event.requestId)) {
    requestIds.set(entry.event.requestId, `mode-change-${String(nextRequest++).padStart(2, '0')}`)
...
console.log(`curated ${curated.length} mode-change lifecycle events to ${destinationPath}`)
