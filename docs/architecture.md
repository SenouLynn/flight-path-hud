...
reflex-sight display. The browser app in [apps/hud](../apps/hud) is not the flight HUD — it is a
...
share the hud app's runtime or build graph.
...
- **Resolvers are pure functions.** Every file in [src/logic/](../apps/hud/src/logic/) exports
...
- **Sanitize at the boundary.** `sanitizeTelemetrySample` (in
  [telemetry.ts](../apps/hud/src/logic/telemetry.ts)) strips `NaN`/`Infinity`/non-numbers to
...
| Path | Responsibility |
|------|----------------|
| [apps/hud/src/logic/telemetry.ts](../apps/hud/src/logic/telemetry.ts) | `TelemetrySample` type + `sanitizeTelemetrySample` boundary guard |
| [apps/hud/src/logic/heading.ts](../apps/hud/src/logic/heading.ts) | Heading source resolution + normalization |
| [apps/hud/src/logic/attitude.ts](../apps/hud/src/logic/attitude.ts) | Pitch/roll → horizon line transform |
| [apps/hud/src/logic/flightPath.ts](../apps/hud/src/logic/flightPath.ts) | Ground speed/climb, 2D track, 3D FPA, predictive path |
| [apps/hud/src/logic/trajectory.ts](../apps/hud/src/logic/trajectory.ts) | Integrated predictive trajectory (blends heading/track/bank/pitch) |
| [apps/hud/src/logic/position.ts](../apps/hud/src/logic/position.ts) | Hybrid position resolver: GPS LLA→ENU with velocity-integration fallback; pure `resolveTrack` fold |
| [apps/hud/src/logic/replay.ts](../apps/hud/src/logic/replay.ts) | Known-answer validation frames + replay runners |
| [apps/hud/src/constants/mavlinkInputs.ts](../apps/hud/src/constants/mavlinkInputs.ts) | Canonical MAVLink field registry (message/field/units/type) |
| [apps/hud/src/stream/telemetrySource.ts](../apps/hud/src/stream/telemetrySource.ts) | Synthetic-replay and live-mock sample generators |
| [apps/hud/src/stream/useTelemetryFeed.ts](../apps/hud/src/stream/useTelemetryFeed.ts) | React hook subscribing to a `TelemetrySource` |
| [apps/hud/src/stream/useFlightTrack.ts](../apps/hud/src/stream/useFlightTrack.ts) | Bounded breadcrumb accumulator over the feed (the only stateful data-path layer) |
| [apps/hud/src/components/](../apps/hud/src/components/) | SVG HUD instruments (see below) |
| [apps/hud/src/App.tsx](../apps/hud/src/App.tsx) | Wires feed + replay tables + instrument previews |

### Components

| Component | Renders |
|-----------|---------|
| [HudPrimaryFlightDisplay](../apps/hud/src/components/HudPrimaryFlightDisplay.tsx) | Composes heading tape over attitude indicator |
| [HudHeadingIndicator](../apps/hud/src/components/HudHeadingIndicator.tsx) | Scrolling compass tape (yaw) |
| [HudAttitudeIndicator](../apps/hud/src/components/HudAttitudeIndicator.tsx) | Artificial horizon + pitch ladder + roll arc |
| [HudOrientationIndicator](../apps/hud/src/components/HudOrientationIndicator.tsx) | 3D wireframe vehicle model (roll/pitch/yaw) |
| [HudPredictiveTrajectory](../apps/hud/src/components/HudPredictiveTrajectory.tsx) | Integrated forward path with drift/stall cues |
| [HudFlightPathRecorder](../apps/hud/src/components/HudFlightPathRecorder.tsx) | Orbitable 3D breadcrumb of accumulated position (ENU world, ground shadow + drop-lines) |
...
- **Two attitude implementations exist and can disagree on sign.**
  [attitude.ts](../apps/hud/src/logic/attitude.ts) drives the replay-preview horizon using a
...
- **NED sign flips.** Velocity `vz` is **positive-down** in MAVLink; climb rate is
  **positive-up**. Resolvers negate (`-vz`) — see [flightPath.ts](../apps/hud/src/logic/flightPath.ts).
