...
Implemented in [src/logic/attitude.ts](../src/logic/attitude.ts). This is the *canonical*
...
[HudAttitudeIndicator.tsx](../src/components/HudAttitudeIndicator.tsx) renders the live HUD
...
[HudOrientationIndicator.tsx](../src/components/HudOrientationIndicator.tsx) draws a
...
`ATTITUDE_VALIDATION_FRAMES` in [replay.ts](../src/logic/replay.ts): level flight,
+10° pitch (→ 60 px), +30° roll, combined down-pitch/left-roll, and a missing-roll frame
that must resolve to `null`. Unit tests: [attitude.test.ts](../src/logic/attitude.test.ts).
