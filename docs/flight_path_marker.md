...
- [src/logic/flightPath.ts](../src/logic/flightPath.ts) — instantaneous vectors + a
  kinematic predictive path (linear vs turn-aware).
- [src/logic/trajectory.ts](../src/logic/trajectory.ts) — a richer integrated trajectory
...
[HudPredictiveTrajectory](../src/components/HudPredictiveTrajectory.tsx). It pulls scalar,
...
`FLIGHT_PATH_VALIDATION_FRAMES` in [replay.ts](../src/logic/replay.ts) pins expected track,
...
[flightPath.test.ts](../src/logic/flightPath.test.ts) and
[trajectory.test.ts](../src/logic/trajectory.test.ts).
